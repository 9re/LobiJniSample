...
static void joinGroupWithExternalID_callback(int code, const char *response, void *userData) {
    ANativeActivity *nativeActivity = (ANativeActivity *)userData;
    if (LOBI_API_SUCCESS == code) {
        // do something
        lobi_showToast(nativeActivity, "joined to the group successfully!");
        lobi_showChatView(nativeActivity);
    } else {
        handle_lobi_error(nativeActivity, code, response);
    }
}

static void signupWithBaseName_callback(int code, const char *response, void *userData) {
    ANativeActivity *nativeActivity = (ANativeActivity *)userData;
    if (LOBI_API_SUCCESS == code) {
        // do something
        lobi_showToast(nativeActivity, "signup success!");
        // join to the group
        lobi_joinGroupWithExternalID(nativeActivity, "111", "aaa", joinGroupWithExternalID_callback, nativeActivity);
    } else {
        handle_lobi_error(nativeActivity, code, response);
    }
}
...
void android_main(struct android_app* state) {
    ...
    if (!lobi_isSignedIn(nativeActivity)) {
        lobi_showToast(nativeActivity, "not signed in!");
        lobi_signupWithBaseName(nativeActivity, signupWithBaseName_callback, nativeActivity);
    } else {
        lobi_showChatView(nativeActivity);
    }
